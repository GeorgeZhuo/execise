...
void BPL::BackPropogation(double *examples) {

    double sum = 0.0;
    // output layer delta
    for (int i = 0; i < OUT_NUM; ++i) {
	delta[1][i] = (target[(int)examples[DIMENSION -1] - 1][i]
		       - output[1][i]) 
...
double BPL::calErr(double *examples) {
    double sum = 0.0;

    for (int i = 0; i < OUT_NUM; ++i) {
	sum += pow((target[(int)examples[DIMENSION - 1] - 1][i] - output[1][i]), 2);
    }
...
//    restoreWeight();
     ...
     double (*learning_examples)[DIMENSION] 
	 = new double[LEARNING_NUM + TEMP_TEST][DIMENSION];
	...
	for (int i = 0; i < LEARNING_NUM; i++) {
	    ...
	    if (calCor() == (int)learning_examples[i][DIMENSION - 1] - 1)
		counter++;

	    BackPropogation(learning_examples[i]);

	    //Err += calErr(learning_examples[i]);
	   
	}

	if (counter > LEARNING_NUM - 50)
	    break;
